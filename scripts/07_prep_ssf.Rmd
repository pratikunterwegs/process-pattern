...
for (path in paths) {
  ...
  data = Map(
    function(df, land) {
      ...
      agents_h = read_landscape(land, layer = c(2), value_name = "handlers")
      agents_nh = read_landscape(land, layer = c(1, 3), value_name = "nonhand")
      df = link_landscape(df, items)
      df = link_landscape(df, agents_h)
      df = link_landscape(df, agents_nh)
